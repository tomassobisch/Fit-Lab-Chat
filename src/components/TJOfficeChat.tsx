...
export const TJOfficeChat: React.FC = () => {
  ...
  const handleSend = async (e: React.FormEvent) => {
        ...
        const generateResponse = async () => {
          ...
          try {
            if (apiKey) {
              ...
              // Failsafe: Timeout de 8 segundos para evitar bloqueos
            ...
            console.error("Fallo de fondo:", aiErr);
          } finally {
            // GARANTÍA: Siempre liberamos el estado de "pensando"
            setIsTyping(null);
          }
        };
    ...
    } catch (err: any) { alert("Mainframe connection lost. Please reload."); }
      ...
      <main className="flex-1 flex flex-col bg-[#050505] overflow-hidden border-r border-white/10 relative">
        <header className="h-16 flex items-center justify-between px-6 border-b border-white/10 bg-black sticky top-0 z-20">
          <div className="flex items-center gap-3">
            ...
            <span className="ml-2 px-1.5 py-0.5 rounded bg-[#CCFF00]/10 text-[#CCFF00] text-[8px] font-black border border-[#CCFF00]/20 uppercase">v1.6-STABLE</span>
          </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 scrollbar-hide">
          {mensajes.length === 0 && !isSyncing ? (
            <div className="h-full flex flex-col items-center justify-center opacity-10"><Terminal size={32} /><p className="mt-4 text-[9px] tracking-[0.5em] font-bold uppercase">Awaiting commands...</p></div>
          ) : (
            mensajes.map(m => (
              <div key={m.id} className={`flex gap-4 max-w-3xl mx-auto animate-in ${m.remitente_tipo === 'agente' ? 'bg-white/5 border border-white/5 p-4 rounded-lg' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] font-bold text-white/60 uppercase">{m.remitente_tipo === 'agente' ? [...INITIAL_AGENTS, ...agentes].find(a => a.id === m.remitente_id)?.nombre : 'Operador'}</span>
        ...
        <div className="p-4 md:p-6 bg-black border-t border-white/10">
          <form onSubmit={handleSend} className="max-w-3xl mx-auto relative">
            <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Escribe al sistema..." disabled={isSending} className="w-full bg-white/5 border border-white/10 rounded-xl py-3.5 pl-5 pr-14 text-base md:text-[12px] text-white focus:outline-none focus:border-[#CCFF00]/50 transition-all" />
            ...
            {ultimoReporte ? (
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-purple-950/40 border border-purple-500/20 shadow-inner">
                  ...
                  <p className="text-3xl font-black text-white">{ultimoReporte.total_alumnos}</p>
                </div>
                <div className="p-4 rounded-xl bg-orange-950/20 border border-orange-500/20">
                  <p className="text-[8px] font-bold text-orange-400 uppercase mb-2 tracking-widest">Pendientes</p>
                ...
                <div className="p-4 rounded-xl bg-red-950/20 border border-red-500/20">
                   <p className="text-[8px] font-bold text-red-400 uppercase mb-2">Escaneos Faltantes</p>
                   <div className="max-h-32 overflow-y-auto space-y-1">
                      {ultimoReporte.pendientes_escaneo.slice(0, 3).map((n, i) => (<div key={i} className="text-[9px] text-red-100 flex justify-between"><span>{n}</span><span className="text-red-500 font-black">!</span></div>))}
                   </div>
                </div>
              </div>
            ) : (
              <div className="p-10 text-center opacity-20"><Activity size={24} className="mx-auto text-purple-400" /><p className="text-[8px] font-bold uppercase mt-2">No Feed</p></div>
            )}
        </div>
      </aside>
    </div>
